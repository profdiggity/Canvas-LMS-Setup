...
"@
    ...
    $dynSettingsFile = Join-Path $configDir "dynamic_settings.yml"
    if (-not (Test-Path $dynSettingsFile)) {
...
development:
  config:
    canvas:
      canvas:
        encryption-secret: "$ltiEncSecret"
        signing-secret: "$ltiSignSecret"
      rich-content-service:
        app-host: "localhost"
"@
        Write-Ok "config/dynamic_settings.yml (generated)"
    } else {
...
"@
    ...
    # .env sets COMPOSE_FILE so users can run plain "docker compose" commands
    ...
    Write-UnixFile (Join-Path $CANVAS_DIR ".env") @"
COMPOSE_FILE=docker-compose.yml:docker-compose.override.yml
"@
    Write-Ok ".env (COMPOSE_FILE set -- plain 'docker compose' commands now work)"
