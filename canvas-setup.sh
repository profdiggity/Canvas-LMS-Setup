...
if "archive.debian.org" in text and "99no-check-valid-until" in text:
    print("  Postgres Dockerfile already patched"); sys.exit(0)
archive_block = (
    'RUN set -eux; \\\n'
...
print("  Patched old PostGIS Dockerfile for EOL Debian archive")
INNERPY
...
#
# encryption_key:      cookie signing + general encryption (20+ chars)
# jwt_encryption_keys: LTI platform storage JWTs - MUST be 64 hex chars each.
#                      Canvas renders an LTI forwarding frame on EVERY page load;
#                      without this the dashboard crashes with RuntimeError.
# lti_iss:             LTI 1.3 issuer identifier. Must match what LTI tools expect.
#                      Default is canvas.instructure.com which works for local dev.
development:
...
EOF
    ...
    # to ensure the file always exists with at least the canvas_security keys needed for
    # LTI encryption. Without lti-encryption-secret/lti-signing-secret some LTI 1.3
    # flows will fail, even in local dev.
    if [[ ! -f "$CANVAS_DIR/config/dynamic_settings.yml" ]]; then
        # Generate two separate 32-char secrets for LTI encryption
        local lti_enc_secret lti_sign_secret
        lti_enc_secret="$(tr -dc 'a-z0-9' < /dev/urandom | head -c 32 || true)"
        lti_sign_secret="$(tr -dc 'a-z0-9' < /dev/urandom | head -c 32 || true)"
        cat > "$CANVAS_DIR/config/dynamic_settings.yml" <<EOF
# Generated by canvas-setup.sh — safe to commit (no real secrets here for local dev)
development:
  config:
    canvas:
      canvas:
      ...
      rich-content-service:
        # RCE (rich text editor) runs as a separate service; not configured here.
        # Canvas falls back gracefully without it for basic course content.
        app-host: "localhost"
EOF
...
build_and_start() {
    ...
    # --- Setup via run --rm (two steps) --------------------------------------
    # Step 1 (root): initialise the named-volume directories with correct
    #   ownership and wipe any stale Yarn cache from a previous failed attempt.
    # Step 2 (docker user, login shell): install gems + assets, seed DB.
    #   bash -lc loads the docker user profile so GEM_HOME, BUNDLE_APP_CONFIG
    #   and PATH are set exactly as Canvas expects.
    #   Named volumes (canvas_gems, canvas_bundle, canvas_cache) persist gems
    #   into the serving containers even after run --rm exits.
...
# Detect the host's real LAN IP (skips loopback and Docker bridge interfaces)
# so the URL works from any device on the same network, not just localhost.
HOST_IP="$(ip -4 addr show scope global     | grep -v 'docker\|br-\|veth'     | grep -oP '(?<=inet\s)\d+(\.\d+){3}'     | head -1)"
HOST_IP="${HOST_IP:-localhost}"

echo "  URL (this machine):   http://localhost:${PORT}"
echo "  URL (network/VM):     http://${HOST_IP}:${PORT}"
